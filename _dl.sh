...
if [ "${_BRANCH#*dev*}" != "${_BRANCH}" ]; then
  ...
  curl -o pack.bin -L --proto-redir =https https://github.com/libssh2/libssh2/archive/e41d6f4b0ea649db3abfdaf45f2dde9979f0b808.tar.gz || exit 1
else
  curl \
...
if [ "${_BRANCH#*dev*}" != "${_BRANCH}" ]; then
  ...
  curl -o pack.bin -L --proto-redir =https https://github.com/curl/curl/archive/748f5301c0a91ad8bab2b74097bc25e3d99317b9.tar.gz || exit 1
else
  curl \
